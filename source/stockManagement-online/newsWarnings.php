<?php
    ...
    if ($result && mysql_num_rows($result) > 0) {
        ...
        $result = mysql_query($sql);
        if ($result && mysql_num_rows($result) > 0) {
            $sql = "SELECT `sv`.`id`, `so`.`id` as `object`, `so`.`title`, `so`.`minimumStock`, `so`.`quotaStock`, `so`.`totalVolume`, `sv`.`date` FROM `StockValue` `sv` INNER JOIN `StockObject` `so` ON (`sv`.`stockObjectId` = `so`.`id`) WHERE `sv`.`messageId` = 2 and `so`.`typeId` = 2;";
            $result = mysql_query($sql);
            if ($result && mysql_num_rows($result) > 0) {
                echo "<div class=\"panel panel-warning\"> \n";
                echo "<div class=\"panel-heading\"> \n";
